...
vector<Person> manualInputPerson()
{
 ...
 vector<Person> inputFile_Vector(string& fileName)
 {
     ...
     while (true)
     {
         if (!file.is_open())
         {
             std::cerr << "\nFile not found! Please enter a valid file name in your work folder: ";
             cin >> fileName;
             file.open(fileName);
         }
             if (file.is_open())
     ...
     cout << "\nTime taken to read to buffer then into a vector containing vector of strings: " << duration.count() << " seconds\n";
 ...
 list<Person> inputFile_List( string& fileName)
 {
	 ...
	 std::chrono::duration<double> duration = high_resolution_clock::now() - start;
	 cout << "\nTime taken to read to buffer then into list container with list of strings: " << duration.count() << " seconds\n";
 ...
 deque<Person> inputFile_Deque(string& fileName)
 {
     ...
     std::chrono::duration<double> duration = high_resolution_clock::now() - start;
     cout << "\nTime taken to read to buffer then into deque container with list of strings: " << duration.count() << " seconds\n";
